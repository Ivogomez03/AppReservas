...
const ModificarAulaMultimedio = () => {
    ...
    const aulaMultimedio = location.state?.aula;
    console.log("El aula es ", aulaMultimedio);
    ...
    useEffect(() => {
        if (aulaMultimedio) {
            setForm({
                numeroDeAula: aulaMultimedio.numeroDeAula || '',
                piso: aulaMultimedio.piso || '',
                capacidad: aulaMultimedio.capacidad || '',
                televisor: aulaMultimedio.televisor || false,
                canion: aulaMultimedio.canion || false,
                computadora: aulaMultimedio.computadora || false,
                tipoPizarron: aulaMultimedio.tipoPizarron || '',
                aireAcondicionado: aulaMultimedio.aireAcondicionado || false,
                ventilador: aulaMultimedio.ventilador || false
            });
        }
    }, [aulaMultimedio]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`/modificarAula`, {
                ...
                body: JSON.stringify({ ...form, tipoAula: aulaMultimedio.tipoAula }),
            });
    ...
    return (
        <div className="containerModAulaM">
            <div className="panel-izquierdo-mod-aula-m">
                ...
                <h1>Modificar Aula Multimedio</h1>
            ...
            <form onSubmit={handleSubmit} className="formularioModAulaM">
                ...
                <div className="checkbox-container">
                    {["televisor", "canion", "computadora", "aireAcondicionado", "ventilador"].map((name) => (
                        <button
                            key={name}
                            type="button"
                            className={`checkbox-btn ${form[name] ? "active" : ""}`}
                            onClick={() => setForm((prevForm) => ({ ...prevForm, [name]: !prevForm[name] }))}
                        >
...
export default ModificarAulaMultimedio;
